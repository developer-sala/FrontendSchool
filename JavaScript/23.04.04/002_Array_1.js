// arr(*****)
const arr = [10, 20, 30];
arr[0] = 100; // const는 값이 변화가 안된다고 하지 않았나? // 변경가능, mutable
// arr = 100
// arr = [10, 20, 30]
console.log(arr);
...
// 그렇기에 인공지능 중 가장 유명한 라이브러리 텐셔플로우는 결국 다차원 행렬의 연산입니다.
...
const arr = [1, 2, 3];
arr.splice(1, 0, 4); // arr 1번째에, 아무것도 삭제하지 않고 4를 넣겠다.

const arr = [1, 2, 3];
arr.splice(1, 0, [10, 20, 30]); // arr 1번쨰에, 아무것도 삭제하지 않고, [10, 20, 30]를 넣겠다.

const arr = [1, 2, 3];
arr.splice(1, 0, 10, 20, 30); // arr 1번째에, 아무것도 삭제하지 않고, 10, 20, 30를 넣겠다.
...
const arr = [10, 20, 30, 40, 50];
arr.splice(2, 1, 5); // arr 2번째에, 1개를 삭제하고, 5를 넣는다.
...
// 같은 코드4(이렇게 사용하지 않습니다. 이름이 있는 함수 사용)
const arr = Array(100).fill(0);
const arr2 = [];
